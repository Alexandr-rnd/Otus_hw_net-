...
with open("request.txt", "w") as txt:# вычитываем заголовки и тело
    ...
    if 'status' in cl_request[0]: #проверяем параметр переданного статус кода
        ...
        if int(status_mask[0]) in list(HTTPStatus) and status_mask[1] in str(list(HTTPStatus)):#проверяем валидность переданного статус кода
...
with open("request.txt", "r") as txt:
    ...
    conn.send(f"HTTP/1.1 {status_code}\n Content-Length: 100\n Connection: close\n Content-Type:"
              f"text/html\n\n<p>Request Method:{cl_request[0].split('/')[0]}</p><p>Request Source:{cl_request[1][7:-1]}</p><p>Request Status:{status_code}</p>{HtmlString}".encode(
        "utf-8"))
my_socket.close()
